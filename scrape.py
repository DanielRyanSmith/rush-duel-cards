...
def parse_card(card_url):
    ...
    info = {}
    try:
        info['card_type'] = soup.find('a', {'title': 'Card type'}).parent.next_sibling.next_sibling.text.strip()
        if info['card_type'] == 'Monster':
            info['monster_types'] = soup.find('a', {'title': 'Type'}).parent.next_sibling.next_sibling.text.strip().split(' / ')
            info['monster_attribute'] = soup.find('a', {'title': 'Attribute'}).parent.next_sibling.next_sibling.contents[1].text
            info['monster_level'] = soup.find('a', {'title': 'Level'}).parent.next_sibling.next_sibling.contents[1].text
            info['monster_attack'] = soup.find('a', {'title': 'ATK'}).parent.next_sibling.next_sibling.contents[1].text
            info['monster_defense'] = soup.find('a', {'title': 'DEF'}).parent.next_sibling.next_sibling.contents[3].text
        else:
            info['card_property'] = soup.find('a', {'title': 'Property'}).parent.next_sibling.next_sibling.text.strip()
        info['card_name'] = soup.find(class_='card-table').find('div').find('div').text
        info['japanese_name'] = soup.find(class_='card-table').contents[3].contents[0].contents[1].contents[6].text
        info['card_status'] = soup.find('a', {'title': 'Status'}).parent.next_sibling.next_sibling.contents[1].text.split(' ')[0]
        info['image_url'] = soup.find(class_='cardtable-main_image-wrapper').contents[0].contents[0].attrs['src']
